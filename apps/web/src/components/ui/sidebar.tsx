...
// open/openMobile are arrays of sidebar names that are currently open.
// state is a legacy derived field ("expanded" if any sidebar is open).
...
function SidebarProvider({
  ...
  const initialOpen = defaultOpen === "all" ? sidebarNames : defaultOpen
  const [_open, _setOpen] = React.useState<string[]>(initialOpen)
  ...
  const setOpen = React.useCallback(
    (value: string[] | ((value: string[]) => string[])) => {
      ...
      document.cookie = `${SIDEBAR_COOKIE_NAME}=${openState.join(",")}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`
    },
    [setOpenProp, open]
  )
  ...
  // Keyboard shortcut toggles the first sidebar name (or 'left' by default).
  React.useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (
        ...
        toggleSidebar([sidebarNames[0] ?? "left"])
      }
    }
  ...
  // Legacy state field: "expanded" if any named sidebar is open.
  const state: "expanded" | "collapsed" = open.length > 0 ? "expanded" : "collapsed"
...
function Sidebar({
  ...
  const isOpenMobile = openMobile.includes(name)
